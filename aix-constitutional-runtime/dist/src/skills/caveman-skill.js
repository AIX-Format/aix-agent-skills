...
// Independent logging
const Logger = {
    debug: (msg) => console.debug(`🦴 [CAVEMAN] ${msg}`),
};
export class CavemanSkill {
     ...
     */
    static deterministicShield(text) {
        if (!text)
            return "";
        return text
     ...
     */
    static rehydratePrompt(compressed) {
...
`;
     ...
     */
    static extractArabicRoots(text) {
     ...
     */
    static compressPrompt(text, mode = 'basic') {
        if (!text)
            return "";
        let compressed = text
            // --- English Compression ---
        ...
        Logger.debug(`${mode.toUpperCase()} Compressed: ${text.length} -> ${compressed.length} chars`);
        return compressed;
    }
    /**
     * Transforms a response into a more 'Sovereign Brevity' style if requested.
     */
    static sovereignBrevity(text) {
